...
FIXME: "Unsupported or nonspecified audio format!"
...
FIXME: Need more explanation what input/output files are?
...
FIXME: add "output-dir" to "> stage1_predictions.txt" in invokation of Ruby script
...
> cp filelist.txt /path/to/workdir/
> ./kea -m distance_matrix_MIREX -id /path/to/workdir -od /path/to/workdir -w filelist.arff -pr filelist.txt -dm filelist_matrix.txt

The generated filelist_matrix.txt contains the full distance matrix
for all songs of filelist.txt

QUESTION: copying filelist.txt to workdir is a bit silly. Remove the use of "-id" option for "kea" instead?
...
FIXME: "pitchextract -h" outputs nothing:
  Reason is duplicate: "--hopsize" also has short form "-h"
