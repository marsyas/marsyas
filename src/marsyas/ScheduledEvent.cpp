...
{
	setEvent(s.time_,s.repetition_,s.event_->clone());
	setTimer(s.timer_);
}
...
void
ScheduledEvent::setEvent(mrs_natural time, MarEvent* event)
{
	time_=time;
	event_=event;
	repetition_=Repeat();
...
void
ScheduledEvent::setEvent(mrs_natural time, Repeat rep, MarEvent* event)
{
	time_=time;
	event_=event;
	repetition_=rep;
...
void
ScheduledEvent::setTimer(TmTimer* t)
{
	timer_=t;
...
mrs_natural
ScheduledEvent::getRepetitionCount()
{
...
//            time_ = time_ + timer_->intervalsize(repetition.interval);
			...
			// otherwise kill it
			repetition_.setInfinite(false);
			repetition_.setCount(0);
		}
	}
}
/*
...
{
	setEvent(a.time_,a.repetition_,a.event_->clone());
	setTimer(a.timer_);
	return *this;
}

bool
ScheduledEvent::operator<(const ScheduledEvent &se) const
{
