	...
	if(!nonCausal_)
	{
		...
		if(in(0,0) == 1.0) //only count after 1st beat
		{
			...
			if(beatCount_ >= inc_)
			{
				...
				if ((mode_ == "beatTimes") || (mode_ == "beats+meanTempo")
				   || (mode_ == "beats+medianTempo") || (mode_ == "beats+meanTempo+medianTempo"))
				{
					ostringstream oss;
				...
				if ((mode_ == "meanTempo") || (mode_ == "beats+meanTempo")
				   || (mode_ == "beats+meanTempo+medianTempo"))
				{
					ostringstream oss2;
					if(initialOut2_)
					{
				...
				if ((mode_ == "medianTempo") || (mode_ == "beats+medianTempo")
				   || (mode_ == "beats+meanTempo+medianTempo"))
				{
					ostringstream oss3;
					if(initialOut3_)
	...
	if(nonCausal_)
	{
		if(t == soundFileSize_-1) //[! -1 for acouting on time of timing reset on backtrace mode]
		{
			...
			if(bestFinalAgentHistory_(0) >= 0.0)
			{
				...
				fstream outStream;
				ostringstream oss;
				if ((mode_ == "beatTimes") || (mode_ == "beats+medianTempo") 
				   || (mode_ == "beats+meanTempo") || (mode_ == "beats+meanTempo+medianTempo"))
				{
					oss << ctrl_destFileName_->to<mrs_string>() << ".txt";
					outStream.open(oss.str().c_str(), ios::out|ios::trunc);
					outStream << beatTime_ << endl;
