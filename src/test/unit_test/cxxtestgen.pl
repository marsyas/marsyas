...
sub scanLineForTest($$) {
  my ($lineNo, $line) = @_;
  # sness - Needed to change this regexp to make it so that we can define a function
  # on the next line like we like to do in Marsyas
#  if ( $line =~ m/^([^\/]|\/[^\/])*\bvoid\s+([Tt]est\w+)\s*\(\s*(void)?\s*\)/ ) {
  if ( $line =~ m/^([^\/]|\/[^\/])*\s+([Tt]est\w+)\s*\(\s*(void)?\s*\)/ ) {
    addTest( $2, $lineNo );
  }
}
