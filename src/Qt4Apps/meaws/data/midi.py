...
Python MIDI classes: meaningful data structures that represent
MIDI
events
and other objects. You can read MIDI files to create such objects,
or
generate a collection of objects and use them to write a MIDI
file.
...
Will Ware <w...@alum.mit.edu>

python Midi.py -i alaala.mid -p > ABC.txt

BLB - 9/22/2006 modifications for XML output
...
# runningStatus appears to want to be an attribute of a MidiTrack.
# But
...
class MidiEvent:
    ...
    def __repr__(self):
        r = ("<MidiEvent type='%s' t='%s' track='%s' channel='%s'" %
             (self.type,
              repr(self.time),
              self.track.index,
              repr(self.channel)))
        ...
        if channelVoiceMessages.has_value(y):
            ...
            if (self.type == "PROGRAM_CHANGE" or
                ...
                if (self.type == "NOTE_OFF" or
                    (self.velocity == 0 and self.type ==
"NOTE_ON")):
                    channel.noteOff(self.pitch, self.time)
                elif self.type == "NOTE_ON":
                    channel.noteOn(self.pitch, self.time,
self.velocity)
                return str[3:]
...
"""
register_note() is a hook that can be overloaded from a script
that
imports this module. Here is how you might do that, if you wanted
to
store the notes as tuples in a list. Including the distinction
between track and channel offers more flexibility in assigning
voices.
...
connecting
    a NOTE_ON event with its corresponding NOTE_OFF event.
Together,
those
    ...
    def noteOff(self, pitch, time):
        if self.pitches.has_key(pitch):
            ...
            register_note(self.track.index, self.index, pitch,
velocity,
                          keyDownTime, time)
            del self.pitches[pitch]
        # The case where the pitch isn't in the dictionary is
        # illegal,
