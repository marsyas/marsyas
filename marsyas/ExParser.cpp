...
void ExParser::LAsgn(ExNode** u) {
		...
		if (la->kind == 53) {
			...
			Expect(8);
			Condition(u);
			*u=do_property(NULL,"Stream."+nm,*u); 
		} else if (la->kind == 5) {
			Name(nm);
			if (StartOf(2)) {
				LAsgnOp(atype);
				Condition(u);
				*u=do_masgn(atype,false,nm,*u); 
			} else if (la->kind == 8) {
				Get();
				Condition(u);
				*u=do_asgn(nm,*u); 
...
void ExParser::Factor(std::string& l, ExNode** u) {
		...
		case 45: {
			...
			Expect(50);
			Exprs(&ts);
			if (fail) { delete c; return; } 
			Expect(50);
			Exprs(&es);
			if (fail) { delete c; delete ts; return; } 
			Expect(44);
...
bool ExParser::StartOf(int s) {
	...
	static bool set[6][58] = {
		{T,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x},
		{x,T,T,T, T,T,T,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, T,x,x,x, x,T,x,x, x,T,x,x, x,x,x,x, x,x,x,x, x,T,x,x, x,x,x,T, T,x,x,x, x,x},
		{x,x,x,x, x,x,x,x, x,x,x,x, x,x,T,T, T,T,T,x, x,T,T,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x},
		{x,x,x,x, x,x,x,T, x,T,T,T, T,T,x,x, x,x,x,T, T,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x},
		{x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,T,T, T,T,T,T, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x},
		{x,T,T,T, T,T,T,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, T,x,x,x, x,T,x,x, x,T,x,x, x,x,x,x, x,x,x,x, x,T,T,x, x,x,x,T, T,T,x,x, x,x}
	};
...
void ExParser::SynErr(int n) {
	...
	switch (n) {
			...
			case 51: s = coco_string_create("\"true\" expected"); break;
			case 52: s = coco_string_create("\"false\" expected"); break;
			case 53: s = coco_string_create("\"Stream\" expected"); break;
			case 54: s = coco_string_create("\"use\" expected"); break;
			case 55: s = coco_string_create("\"load\" expected"); break;
			case 56: s = coco_string_create("??? expected"); break;
			case 57: s = coco_string_create("invalid AddOp"); break;
			case 58: s = coco_string_create("invalid MulOp"); break;
			case 59: s = coco_string_create("invalid RelOp"); break;
			case 60: s = coco_string_create("invalid LAsgnOp"); break;
			case 61: s = coco_string_create("invalid RAsgnOp"); break;
			case 62: s = coco_string_create("invalid Task"); break;
			case 63: s = coco_string_create("invalid LAsgn"); break;
			case 64: s = coco_string_create("invalid LAsgn"); break;
			case 65: s = coco_string_create("invalid LAsgn"); break;
			case 66: s = coco_string_create("invalid Link"); break;
			case 67: s = coco_string_create("invalid RAsgn"); break;
			case 68: s = coco_string_create("invalid RAsgn"); break;
			case 69: s = coco_string_create("invalid RAsgn"); break;
			case 70: s = coco_string_create("invalid Factor"); break;
			case 71: s = coco_string_create("invalid UL"); break;
