...
{
  if ( control.rfind( "real", control.length()) != string::npos ) 
    {
      if ( workingSet.find(name) != workingSet.end() ) {
	real val = (real)atof( value.toString().c_str() );
  ...
  else if ( control.rfind( "string", control.length() ) != string::npos ) {
  ...
  else if ( control.rfind( "natural", control.length()) != string::npos ) {
    if ( workingSet.find(name) != workingSet.end() ) {
      natural val = atoi( value.toString().c_str() );
      workingSet[name]->updctrl( control, val );
      return true;
    } else {
  ...
  else if ( control.rfind( "bool", control.length()) != string::npos ) {
...
bool MslCommandRun::execute()
{
  ...
  while (true) {
	...
	if ( controls != 0 ) {
		...
		natural inSamples = msys->getctrl("natural/inSamples").toNatural();
		natural inObservations = msys->getctrl("natural/inObservations").toNatural();
		real israte = msys->getctrl("real/israte").toReal();
			
		if ( (natural)controls[1] != inSamples || (natural)controls[2] != inObservations 
				|| controls[3] != israte ) {
		
			msys->updctrl("natural/inSamples",(natural) controls[1]);
			msys->updctrl("natural/inObservations", (natural)controls[2]);
			msys->updctrl("real/israte", controls[3]);
		}
      	}
