...
MarTemplateBasic::addControls()
{
	...
	addControl("mrs_bool/dummy", false);
...
MarTemplateBasic::myProcess(realvec& in, realvec& out)
{
  ...
  //
	...
	// this is the way to do it (in this case we should let it call update(),
	// so we do not use the NOUPDATE flag):
	ctrl_repeats_->setValue(++repeats%4 + 1); //just cycle the number of repeats: 1, 2, 3, 4, 1, 2, 3... 
}
