...
#########################################
# FIXED BY Marcos Domingues and Fabien Gouyon
# Date: March 25, 2011
# Comments: The code below is commented because it does not work correctly. For the predict file, it does not consider the case where the clips have no tags predicted/attributed, ignoring such clips. Following, we suggest a fixed version for such a code.
#########################################
...
ground_truth_file = File.open(ARGV[0],"r")
ground_truth = {}
predict = {} # FIXED: predicted tags structure
begin
  ground_truth_file.each_line do |line|
    a = line.chomp.split("\t")
    clip = a[0]
    ...
    if ground_truth[clip].nil?
      ground_truth[clip] = {}
      predict[clip] = {} # FIXED: create the predicted tags structure for each clip
      tags.each do |k,v|
        ground_truth[clip][k] = 0
        predict[clip][k] = 0 # FIXED: initialize the predicted tags structure for each clip and tag
      end
    end
    ground_truth[clip][tag] = 1
  end
...
begin
  predict_file.each_line do |line|
    ...
    clip = a[0]
    tag = a[1]
# FIXED: Removed this code (initialization of the predicted tags structure was carried out in the previous code)
#    if predict[clip].nil?
#      predict[clip] = {}
#      tags.each do |k,v|
#        predict[clip][k] = 0
#      end
#    end
    if predict[clip].nil? # FIXED: clip without tags
    else
      predict[clip][tag] = 1 # FIXED: clip with a given tag
    end
  end
rescue EOFError
  predict_file.close
...
# Variables to calculate global measures
...
tags.each do |tag,z|
  ...
  # Store values to calculate global measures
...
global_precision = global_true_positives.to_f / (global_true_positives + global_false_positives)
global_recall = global_true_positives.to_f / (global_true_positives + global_false_negatives)
global_accuracy = (global_true_positives.to_f + global_true_negatives) / (global_true_positives.to_f + global_true_negatives + global_false_positives + global_false_negatives)
global_f_score = (2.0 * (global_precision * global_recall)) / (global_precision + global_recall)

puts "Per Tag Values"
puts "Precision, Recall, Accuracy, F_Score"
puts "%5.3f %5.3f %5.3f %5.3f" % [avg_precision, avg_recall, avg_accuracy, avg_f_score]
puts "\n"
puts "Global Values"
puts "Precision, Recall, Accuracy, F_Score"
puts "%5.3f %5.3f %5.3f %5.3f" % [global_precision, global_recall, global_accuracy, global_f_score]
